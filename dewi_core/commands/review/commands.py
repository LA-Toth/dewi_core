...
def register_global_args_in_review_cmd(c: ClickHelper) -> None:
    """
...
class AutoReviewSubCommand(Command):
    def run(self, ctx: ApplicationContext) -> typing.Optional[int]:
        ...
        print("Existing repo?: ", os.path.exists(ctx.args.gerrit_review_local_repo))
        self.args = ctx.args
        return self._run()
...
class GerritChangeReviewer(AutoReviewSubCommand):
    ...
    @staticmethod
    def register_arguments(c: ClickHelper):
        c.add_option('--gerrit-url', 'gerrit_url',
